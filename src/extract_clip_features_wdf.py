...
# ================= CONFIG =================
FRAMES_ROOT = "data/processed_wdf/frames_grouped"
OUTPUT_ROOT = "data/processed_wdf/clip_features"
...
def process_split(split):
    ...
    dataset = CLIPFrameDataset(
        frames_root=os.path.join(FRAMES_ROOT, split),
    ...
    with torch.no_grad():
        ...
        for idx, (frames, label) in enumerate(
            tqdm(loader, desc=f"Extracting CLIP features ({split})")
        ):
